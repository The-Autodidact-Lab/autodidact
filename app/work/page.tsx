...
function WorkItemComponent({ item, isLast }: WorkItemProps) {
  ...
  const TitleComponent = item.link ? (
    isInternalLink ? (
      <Link
        href={item.link}
        className="group inline-flex items-center gap-1.5 text-lg font-normal text-gray-900 transition-colors hover:text-gray-700 sm:text-lg"
      >
  ...
  return (
    <div className="relative flex gap-4 sm:gap-6">
      ...
      <div className="flex flex-col items-center pt-2">
        ...
        {!isLast && (
          <div className="mt-1 h-32 w-0.5 bg-gray-200 sm:h-26" />
        )}
      </div>
      ...
      <div className="flex-1 pb-6 last:pb-0">
        <div className="space-y-2">
          {TitleComponent}
          <InlineMarkdown content={item.description} />
          {item.author ? (
